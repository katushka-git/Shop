...
namespace MyReklama.Logic
{
    ...
    public class Statistic
    {
...

        {
            ...
            MyService.serviceList = db.Orders.GroupBy(item1 => item1.Service).Select(item => new ServiceItemView() { Name = item.Key.Name, Count = item.Count() }).OrderBy(X=>X.Count).ToList();
